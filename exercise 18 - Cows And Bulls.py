...
def guess():
	gus_num = int(input('Please enter a four digit number you guess:'))
	gus_str = str(gus_num)
	print ('The number you enter is %d'%(gus_num))
	cow_and_Bull (gus_str,set_str)

def cow_and_Bull (gus_str,set_str):
	cow_count=0
	bull_count=0
	global count
	count+=1
	for x in range(len(set_str)):
		if gus_str[x] == set_str[x]:
			cow_count+=1
		elif gus_str[x] in set_str:
			bull_count+=1
	if cow_count==4:
		print ('Congratulations! 4 cows, you win! You have guessed %d time(s)' % (count))
	else:
		count+=1
		print ('%s cow(s), %s bull(s). Please try again.'%(cow_count,bull_count))
		guess()

if __name__ == '__main__':
	print ('Welcome to the Cows and Bulls Game!' )
	print ('Now I\'m generating a four digit number!' )
	set_num=random.randint(1000,9999)
	set_str = str(set_num)
	print (set_num)
	count=0
	guess()